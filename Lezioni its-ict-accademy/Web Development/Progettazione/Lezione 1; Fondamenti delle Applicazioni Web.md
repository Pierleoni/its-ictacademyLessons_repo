...
1. **Client Layer (Presentazione)**
    ...
    - Può essere implementato come:
        ...
        - un’applicazione web in un browser (HTML, CSS, JavaScript),
    ...
    👉 È il livello **superiore**: il punto di contatto tra l’utente e il sistema.
    

2. **Logic Layer (Applicativo o Business Logic)**
    ...
    - Può **aggiungere, eliminare o modificare dati** nel Data Layer.
        
    - È sviluppato in linguaggi come **Python, Java, PHP, Perl, Ruby**, e comunica con il database tramite **API o librerie dedicate**.
        
    
    👉 È il livello **intermedio**, il “cervello” del sistema.
    

3. **Data Layer (Persistenza dei dati)**
    
    - Ha il compito di **gestire e archiviare in modo permanente i dati** dell’applicazione.
        
    - Interagisce soltanto con il Logic Layer (non direttamente con l’utente).
        
    - Può essere realizzato con database:
        
        - **Relazionali**: PostgreSQL, MySQL, MariaDB, Oracle, SQL Server…
...
- **Cliente (Client)** → Sei tu seduto al tavolo. Hai bisogno di qualcosa (cibo o bevande) e fai una **richiesta** al cameriere (API).  
...
3. **Response (Risposta)** → Il server invia la risposta al client.  
    _(Esempio: la pagina HTML, un file JSON, o un messaggio di errore)._
...
- I server hanno indirizzi numerici univoci chiamati **indirizzi IP** (es. `142.250.184.142`).
...
Per capire meglio come lavora il DNS vediamo il flusso di richiesta: 
...
#### Indirizzo IP
Un **indirizzo IP (Internet Protocol)** è un’etichetta numerica che identifica in modo univoco un dispositivo (computer, smartphone, server, ecc.) all’interno di una rete.  
...
📌 I dispositivi di casa usano IP privati e condividono un unico **IP pubblico** grazie al router, che usa il **NAT (Network Address Translation)**.
